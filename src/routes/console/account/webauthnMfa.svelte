...
<Modal title="Add security key" bind:show={showWebauthnSetup} {error}>
    <div class="u-text-center">
        ...
        {:else if step === 2}
            {#if !error}
                <p>Security key added successfully.</p>
            {:else}
                <p>Something went wrong while creating a security key. Please try again.</p>
            {/if}
        {/if}
    </div>
    ...
    <svelte:fragment slot="footer">
        ...
        {#if step === 2}
            <Button on:click={() => (showWebauthnSetup = false)}>Finish</Button>
        {/if}
    </svelte:fragment>
</Modal>
