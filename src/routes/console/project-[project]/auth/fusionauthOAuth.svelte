<script lang="ts">
    ...
    let FusionAuthDomain: string = null;
    let error: string;

    onMount(() => {
        appId ??= provider.appId;
        enabled ??= provider.enabled;
        if (provider.secret) ({ clientSecret, FusionAuthDomain } = JSON.parse(provider.secret));
    });
    ...
    $: secret =
        clientSecret && FusionAuthDomain
            ? JSON.stringify({ clientSecret, FusionAuthDomain })
            : provider.secret;
</script>
...
<Modal {error} size="big" show onSubmit={update} on:close>
    ...
    <FormList>
        ...
        <InputText
            ...
            placeholder="Your FusionAuth domain"
            bind:value={FusionAuthDomain} />
        <Alert type="info">
    ...
    <svelte:fragment slot="footer">
        ...
        <Button
            disabled={(secret === provider.secret &&
                enabled === provider.enabled &&
                appId === provider.appId) ||
                !(appId && clientSecret && FusionAuthDomain)}
