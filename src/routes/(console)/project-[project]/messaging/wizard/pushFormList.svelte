...
<div class="u-flex u-gap-24">
    ...
    <div class="u-colum-gap-2">
        ...
        <Modal title="Send test message" bind:show={showTest} onSubmit={sendTestMessage} size="l">
            <slot />
            <InputRadio
                label={$user.phone}
                bind:group={selected}
        ...
        <div class=" u-grid u-gap-8">
            <ul class="form-list" style="--p-form-list-gap: 1rem">
                {#each customData || [] as _, rowIndex}
                    <InputText
                        id={`${rowIndex}-key`}
                        bind:value={$messageParams[MessagingProviderType.Push].data[rowIndex][0]}
                        placeholder="Enter key"
                        label="Key" />

                    <InputText
                        id={`${rowIndex}-value`}
                        bind:value={$messageParams[MessagingProviderType.Push].data[rowIndex][1]}
