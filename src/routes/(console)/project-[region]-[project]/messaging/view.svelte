                ...
                {#each $columns as column (column.id)}
                    <Table.Cell column={column.id} {root}>
                        ...
                        {:else if column.type === 'datetime'}
                            {#if !message[column.id]}-{:else}{toLocaleDateTime(
                                    message[column.id]
                                )}{/if}
                        {:else}
                            {message[column.id]}
    ...
    <Empty single target="message" on:click={() => ($showCreate = true)}>
        <svelte:fragment slot="actions">
            <Button
                ...
                ariaLabel={`create message`}>Documentation</Button>
            {#if $canWriteMessages}
                <CreateMessageDropdown let:toggle>
                    <Button secondary on:click={toggle} event="create_message"
                        ><span class="text">Create message</span></Button>
                </CreateMessageDropdown>
            {/if}
        </svelte:fragment>
    </Empty>
...
<Confirm title="Delete messages" bind:open={showDelete} onSubmit={handleDelete} disabled={deleting}>
    <Typography.Text
        ...
        {selected.length > 1 ? 'messages' : 'message'}?</Typography.Text>
</Confirm>
