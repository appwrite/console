...
<script lang="ts">
    ...
    const types = $derived(
        [
            ...
            if (type.value === TablesDBIndexType.Spatial && !$regionalConsoleVariables?.supportForSpatials)
                return false;
            return true;
        })
    );
    ...
    $effect(() => {
        ...
        if (selectedType === TablesDBIndexType.Spatial && firstField && !isSpatialType(firstField)) {
