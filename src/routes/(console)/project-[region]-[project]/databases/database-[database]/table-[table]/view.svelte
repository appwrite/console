<script lang="ts">
    ...
    type Props = {
        data: {
            ...
            rows: Models.RowList<Models.DefaultRow>;
    ...
    const selected = $derived(preferences.getCustomTableColumns(page.params.table));
    ...
    const hasColumns = $derived(!!$table.columns.length);
    const hasValidColumns = $derived($table?.columns?.some((col) => col.status === 'available'));
    ...
    <div class="databases-spreadsheet">
        {#if hasColumns && hasValidColumns}
            {#if data.rows.total}
                <Divider />
                <SpreadSheet
                    data={data as unknown as import('./$types.js').PageData}
                    bind:showRowCreateSheet={$showRowCreateSheet} />
