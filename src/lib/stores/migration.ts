...
export const migrationFormToResources = (
    ...
    if (formData.messaging.root) {
        addResource(AppwriteMigrationResource.Provider);
        addResource(AppwriteMigrationResource.Topic);
        addResource(AppwriteMigrationResource.Subscriber);
    }
    if (formData.messaging.messages) {
        addResource(AppwriteMigrationResource.Message);
    }
...
export const resourcesToMigrationForm = (resources: MigrationResource[]): MigrationFormData => {
    ...
    if (includesAll(resources, [AppwriteMigrationResource.Table, AppwriteMigrationResource.Column, AppwriteMigrationResource.Row] as MigrationResource[])) {
        formData.databases.rows = true;
    }
    if (includesAll(resources, [AppwriteMigrationResource.Bucket, AppwriteMigrationResource.File] as MigrationResource[])) {
        formData.storage.root = true;
    }
    if (resources.includes(AppwriteMigrationResource.Provider)) {
        formData.messaging.root = true;
    }
    if (resources.includes(AppwriteMigrationResource.Message)) {
