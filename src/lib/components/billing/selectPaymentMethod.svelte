    ...
    {#if filteredMethods?.length}
        {#if selectedPaymentMethod?.country?.toLowerCase() === 'in'}
            <Alert.Inline status="warning">
                ...
                To comply with RBI regulations in India, we will ask for verification to charge up to
                $150 USD on your payment method. We will never charge more than the cost of your plan
                and the resources you use, or your budget cap limit. For higher usage limits, please
                contact us.
            </Alert.Inline>
        {/if}
        <InputSelect
