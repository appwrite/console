<script lang="ts">
    ...
    function select(
        ...
        if (typeof value === 'string') {
            ...
            const typedField = field as 'service' | 'resource' | 'action';
            const newValue = selected[typedField]?.name === value.name ? null : value;
            (selected[typedField] as EventService | EventResource | EventAction | null) = newValue;
        }
        ...
        switch (field) {
            ...
            case 'resource': {
                ...
                if (!availableAction) {
                    ...
                    if (!selected?.action?.attributes?.includes(selected.attribute as string)) {
                        selected.attribute = null;
                    }
                }
                break;
    ...
    function resetSelected() {
        objectKeys(selected).forEach((key) => (selected[key] = null));
    }
    ...
    let customInput: string | null = null;
